...
* RETURN		: 
******************************************************************************/ 
unsigned char *desEncryptBuf2(unsigned char *srcBuf, int srcLen, int *destLen, int type)
{
	int ret = 0;
	int dstLen = 0;
	int retLen = 0;
	...
	memcpy(hashKey, ENCYRPT_KEY_DEVID, ENCRYPT_KEY_LEN);
	MSG_PRINT("des %scrypt key is %s.\n", DES_ENCRYPTION == type? "en" : "de", hashKey);
	...
	Des_SetKey(&enc, hashKey, iv, DES_ENCRYPTION);
    Des_CbcEncrypt(&enc, dstBuf, srcBuf, srcLen);

	*destLen = dstLen;
/*    Des_SetKey(&dec, hashKey, iv, DES_DECRYPTION);
...
int desEncryptFile(char *srcFileName, char *dstFileName, int type)
{
	...
	dstBuf =  desEncryptBuf2(srcBuf, srcLen, &dstLen, 
							CLOUD_DES_ENCRYPT == type ? DES_ENCRYPTION : DES_DECRYPTION);
	if (0 >= dstLen)
	{
